---
description: Pick and implement the next item from todo.txt
allowed-tools: [Read, Edit, Write, Bash, Agent]
---

# Todo
Each subfolder has a `todo.txt` file.

1. Read a single `todo.txt` file.
2. Pick the single most actionable top-level item (or sub-item if the parent is a group).
3. Implement it fully — explore the codebase as needed, make all required changes.
4. Move **only that item** to the `Done:` section at the bottom of `todo.txt` (create it if it doesn't exist). Leave everything else untouched.
5. Update the relevant -build-spec.json file to reflect the changes you made (if applicable).
6. From that subfolder, run `task install` to build and push the up-to-date debug APK to the connected phone (requires a device reachable via `adb`).
7. Report what you did in 2–3 sentences.
